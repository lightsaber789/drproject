...
os.chdir('/src/rfir/project/aptos2019-blindness-detection')
...
from .augmentation import (InplacePngTransforms, MediumNpyTransforms, HeavyNpyTransforms, KaggleLeaderAugmentation,TensorOnlyTransforms,
                           MixupNpyTransforms)
...
#TODO uncomment this if 3channels
    def __init__(self, data_dir, batch_size, epoch_size, validation_split, num_workers, img_size,
                 train=True, alpha=None, verbose=0, pseudo_dir=None, six_channel=True):
        if six_channel:
            train_tsfm = TensorOnlyTransforms(True)
            test_tsfm  = TensorOnlyTransforms(False)
        else:
        ...
        #train_tsfm = HeavyNpyTransforms(True, img_size)
        #test_tsfm = MediumNpyTransforms(False, img_size)
        dataset = NpyDataset(data_dir, train_tsfm, test_tsfm, train=train, pseudo_dir=pseudo_dir)
