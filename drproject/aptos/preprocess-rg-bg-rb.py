...
PROCESS_DIR = Path('/src/project/aptos2019-blindness-detection/data/preprocessed/rg')
...
def extract_rg(img):
    """Return RG channels (Red + Green) from RGB image."""
    return img[:, :, [0, 1]]  # Keep Red and Green

def extract_gb(img):
    """Return GB channels (Green + Blue) from RGB image."""
    return img[:, :, [1, 2]]  # Keep Green and Blue

def extract_rb(img):
    """Return RB channels (Red + Blue) from RGB image."""
    return img[:, :, [0, 2]]  # Keep Red and Blue
...
with ThreadPoolExecutor(max_workers=NWORKERS) as executor:
    ...
    for future in tqdm(as_completed(results), total=len(train_imgs)):
        try:
            ...
            f = results[future]
            img_rg = extract_rg(img)
            save_as = PRO_TRAIN_DIR / f.stem
            np.save(save_as, img_rg)
