...
sys.path.append('/src/project/aptos2019-blindness-detection')

from aptos.data_loader import ImgProcessor


DATA_DIR = Path('/src/project/aptos2019-blindness-detection/data/raw')
...
# rgb and hsv mix rg bg hs hv rb sv channels, creates 6 channels. check model EffNetOLD. result 0.78
def convert_to_6ch_from_rgb(img):
...
# concatenated rgb hsv. gave result 0.76, model: EffNetHSV
...
# submitted on kaggle. gave result 0.74 for weights 1.	Green & Hue Boosted (Most evidence-based) - [1.0, 2.0, 1.0, 2.0, 1.0, 0.5]
# 2.	Hue Dominant (hemorrhages/exudates color separation matters most) [0.5, 1.0, 0.5, 3.0, 1.0, 0.5]
def concat_6channel_WEIGHTS_with_CLAHE_HSV(img, input_range=(0, 255), weights=None):
    if img.shape[0] == 3:
...
# score 0.760608. concatenated rgb with ycbcr. with weights Green‐heavy bolsters blood‐vessel contrast (critical for microaneurysms and vessel abnormalities), with moderate Y for optic‐disc clarity. Vessel & Optic Disk Focus [0.5,2.0,0.5,1.0,0.5,0.5]
...
PROCESS_DIR = Path('/src/project/aptos2019-blindness-detection/data/preprocessed/psixchannels')
